...
var Server = (function (_super) {
    ...
    Server.prototype.loadLayer = function (layer, done) {
        ...
        var img = new png.Image().on('parsed', function (data) {
            ...
            console.log("DEBUG: loadLayer: on parsed new img DONE");
        });
        switch (this.dataMode) {
            case 1 /* FS */:
    ...
    Server.prototype.ioConnectionListener = function (socket) {
        ...
        socket.on('setRoom', function (room) {
            console.log("DEBUG: setRoom " + room.room);
        });
        socket.on('client', function (newClient) {
            ...
            if(_this.resources[newClient.room].synced == undefined || _this.resources[newClient.room].synced == false) {
                _this.syncLayers(newClient.room, function (){ console.log("DEBUG: ===== syncLayers"); });
                _this.resources[newClient.room].synced = true;
            }
            ...
            if(_this.resources[client.room] == undefined) {
                _this.resources[client.room] = _this.resource;
            }

            //// Redis
            if (_this.dataMode === 2 /* Redis */) {
                _this.initRedisClients();
            }
            // FS
            ...
            _this.sendSystemMessage(util.format('! %s has join room %s.', client.name, client.room));
