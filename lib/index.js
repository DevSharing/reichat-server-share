...
var Server = (function (_super) {
    ...
    function Server(config) {
        ...
        // finally, get sync.
        //this.syncLayers(function () { console.log("DEBUG: ++++++DONE++++++++"); return _this.emit('ready'); });
        this.startDone(function () { return _this.emit('ready'); });
    }
    Server.prototype.listen = function (port, hostname, backlog, callback) {
        console.log("DEBUG: SERVER function listen");
        var _this = this;
    ...
    Server.prototype.startDone = function (done) {
        setImmediate(done);
    }
    Server.prototype.initLayers = function (room) {
        var i, layer;
        for (i = 0; i < this.config.layerCount; i++) {
            layer = new Layer(this.config.canvasWidth, this.config.canvasHeight, i);
            switch (this.dataMode) {
                case 1 /* FS */:
                    layer.path = path.join(this.config.dataDir, [this.config.dataFilePrefix, room, 'layer', i, '.png'].join(''));
                    break;
                case 2 /* Redis */:
                    layer.path = this.config.redisKeyPrefix + '-' + room + '-' + 'layer:' + i;
                    break;
            }
            ///this.resource.layers.push(layer);

            //this.resources[room] = this.resource;
            if(this.resources[room] == undefined) {
                this.resources[room] = {
                    layers: [],
                    clients: []
                };
            }
            this.resources[room].layers.push(layer);
        }
    };
    Server.prototype.syncLayers = function (room, done) {
        var _this = this;
        //var count = this.resource.layers.length;
        var count = this.resources[room].layers.length;

        //this.resource.layers.forEach(function (layer) {
        this.resources[room].layers.forEach(function (layer) {
    ...
    Server.prototype.initFileSystem = function (room) {
    ...
    Server.prototype.initRedisClients = function (room) {
        var _this = this;
    ...
    Server.prototype.subscribeRedis = function (room) {
        var _this = this;
        ...
        this.interval.redisPinging = setInterval(function () {
            var otherServers = [];
            _this.resources[room].clients.forEach(function (client) {
            ...
            setTimeout(function () {
                ...
                _this.resources[room].clients = _this.resources[room].clients.filter(function (client) {
                    return otherServers.indexOf(client.server.id) === -1;
                });
                _this.io.to(room).emit('clients', _this.distributableClients);
            ...
            case 'collect':
                if (data.target === 0 /* Clients */) {
                    this.publishRedis('provide', {
                        target: 0 /* Clients */,
                        body: this.resources[data.client.room].clients.filter(function (client) { return client.server.id === _this.id; })
                    });
                }
                break;
            case 'provide':
                if (data.target === 0 /* Clients */) {
                    this.updateClientsByServer(data.server, data.client.room, data.body);
    ...
    Server.prototype.updateClientsByServer = function (server, room, clients) {
        var _this = this;
        var i;
        for (i = 0; i < this.resources[room].clients.length; i++) {
            if (this.resources[room].clients[i].server.id === server.id) {
                this.resources[room].clients.splice(i, 1);
                i--;
            }
        }
        clients.filter(function (client) { return client.server.id === server.id; }).forEach(function (client) {
            _this.resources[room].clients.push(client);
        });
    };
    Server.prototype.httpServerRequestListener = function (req, res) {
        ...
        */
        else if (/^\/layers\/[a-zA-Z0-9\-]+\/[0-9]+$/.test(location) === true) {
            
            var arrParams = location.split('/');
            
            var room = arrParams[2];
            var layerNumber = arrParams[3];
            //var layerNumber = parseInt(location.match(/^\/layers\/([0-9]+)$/)[1], 10);
            if (layerNumber >= this.config.layerCount) {
                httpUtil.responseError(res, 404);
            }
                ...
                if (req.method === 'HEAD') {
                    ...
                    if(this.resources[room] && this.resources[room].layers[layerNumber]) {
                        
                        this.resources[room].layers[layerNumber].toPngStream(res);
                    }
    ...
    Server.prototype.ioConnectionListener = function (socket) {
        ...
        //this.syncLayers(function () { return console.log("DEBUG: ---syncLayers---"); });
        ...
        //client.room = Math.floor((Math.random() * 2) + 1);
        socket.once('disconnect', function () {
            if (client !== null) {
                if (client.uuid && _this.map.socket[client.uuid]) {
                    delete _this.map.socket[client.uuid];
                    //delete _this.resource.clients[client.uuid];
                    _this.resource.clients.forEach(function(obj, index){
                        if(obj.uuid == client.uuid) {
                            delete _this.resource.clients[index];
                            return;
                        }
                    });
                ...
                if (_this.dataMode === 2 /* Redis */) {
                    if(_this.resources && client.room !== undefined) {
                        _this.publishRedis('provide', {
                            target: 0 /* Clients */,
                            body: _this.resources[client.room].clients.filter(function (client) { return client.server.id === _this.id; })
                        });
                    }
                }
                _this.io.to(client.room).emit('clients', _this.distributableClients);
                _this.sendRoomMessage(client.room, util.format('! %s has left.', client.name));
        ...
        socket.on('client', function (newClient) {
            ...
            if (!newClient.room || newClient.room.length > 30) {
                return;
            }

            socket.join(newClient.room);
            _this.initLayers(newClient.room);
            ...
            if (newClient.uuid && _this.map.client[newClient.uuid] && _this.map.client[newClient.uuid].pin === newClient.pin) {
                ...
                _this.resources[newClient.room].clients.push(client);
            }
            client.name = newClient.name;
            client.room = newClient.room;
            client.remoteAddr = remoteAddr;
            client.isOnline = true;
            _this.map.socket[client.uuid] = socket;


            //// INIT RESOURCES FOR ROOM
            if(!_this.resources[client.room] || _this.resources[client.room] == undefined) {
                _this.resources[client.room] = {layers: [], clients: []};
            }

            //// Redis
            if (_this.dataMode === 2 /* Redis */) {
                _this.initRedisClients(client.room);
            }
            // FS
            if (_this.dataMode === 1 /* FS */) {
                _this.initFileSystem(client.room);
            }
            ...
            _this.io.to(client.room).emit('clients', _this.distributableClients);
            _this.sendRoomMessage(client.room, util.format('! %s has join room %s.', client.name, client.room));
            util.log(util.format('%s %s joined. client=%s<%s>', remoteAddr, socket.id, client.name, client.uuid));

            if(_this.resources[newClient.room].synced == undefined || _this.resources[newClient.room].synced == false) {
                _this.syncLayers(newClient.room, function (){ _this.resources[newClient.room].synced = true; });
            }
        });
    ...
    Server.prototype.canvasToPng = function () {
        ...
        var i, j, l, x, y, a, w = this.config.canvasWidth, h = this.config.canvasHeight, layers = 3;
    ...
    Server.prototype.sendPaint = function (client, paint) {
        ...
        new png.Image().parse(paint.data, function (err, img) {
            ...
            var i, j, x, y, aA, bA, xA, w = _this.config.canvasWidth, h = _this.config.canvasHeight, px = paint.x, py = paint.y, pw = Math.min(paint.x + img.width, w), ph = Math.min(paint.y + img.height, h), iw = img.width, ih = img.height, layer = _this.resources[client.room].layers[paint.layerNumber];
            for (y = py; y < ph; y++) {
                for (x = px; x < pw; x++) {
                    i = (w * y + x) << 2;
                    j = (iw * (y - py) + (x - px)) << 2;
            ...
            if (_this.map.socket[client.uuid]) {
                _this.map.socket[client.uuid].broadcast.to(client.room).emit('paint', ioMessage);
                setImmediate(function () { return _this.map.socket[client.uuid].emit('painted'); });
            }
            else {
                _this.io.to(client.room).emit('paint', ioMessage);
            }
    ...
    Server.prototype.sendStroke = function (client, stroke) {
        ...
        if (this.map.socket[client.uuid]) {
            this.map.socket[client.uuid].volatile.broadcast.to(client.room).emit('stroke', ioMessage);
        }
        else {
    ...
    Server.prototype.sendPointer = function (client, pointer) {
        ...
        if (this.map.socket[client.uuid]) {
            this.map.socket[client.uuid].volatile.broadcast.to(client.room).emit('pointer', ioMessage);
        }
        else {
            Object.keys(this.io.sockets.connected).forEach(function (socketId) {
    ...
    Server.prototype.clearCanvas = function (client) {
        ...
        img.on('end', function () {
            ...
            _this.resources[client.room].layers.forEach(function (l, i) {
                _this.sendPaint({
                    uuid: '0',
                    server: client.server,
                    name: client.name,
    ...
    Server.prototype.sendChat = function (client, chat) {
        ...
        this.io.to(client.room).emit('chat', ioMessage);
    ...
    Server.prototype.sendRoomMessage = function (room, message, server) {
        ...
        this.io.to(room).emit('chat', ioMessage);
    };
